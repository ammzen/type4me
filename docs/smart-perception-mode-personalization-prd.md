...
场景策略建立在统一的“基础润色策略”之上。基础润色策略明确包含：
...
基础润色策略默认不添加标题、编号、列表、称呼、落款或用户未说出的内容。只有场景信号和口述内容本身都明确支持时，才允许进行必要的分段或轻度结构化。

| 场景 | 默认润色策略 |
|---|---|
| 即时聊天 | 简短自然，少用标题和列表，保留口语感 |
| 邮件正文 | 句子完整、礼貌清晰，但不擅自补主题、称呼和落款 |
| 文档与笔记 | 允许自然分段；多要点时适度结构化 |
