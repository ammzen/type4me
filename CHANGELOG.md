...
## v2.1.0 — 设置重构、Intelli Sense 与语音改口 (2026-08-18)

### 新功能
- **全新设置体验与多快捷键**：重构首页、模式、历史、词汇和模型设置；同一模式可配置多个 hold/toggle 快捷键
- **Intelli Sense**：结合 App、控件和有限上下文安全润色，并学习稳定的用户纠错、表达与列表偏好
- **Ask Anything 会话历史**：支持搜索、恢复、重命名、删除会话和继续语音追问
- **统一翻译模式**：自动识别源语言，支持 18 种目标语言并验证输出语言
- **语音改口 Revise**：对最近一次 Type4Me 输入进行受控修改、撤销和失败恢复

### 改进与修复
- 新增盘古空格、直角引号等最终文本格式化选项
- 修复火山 ASR 修订快照重复、Fn 快速点按、登录项和内部剪贴板历史问题
- 复用 LLM client、释放不活跃设置页资源并压缩运行时缓存
